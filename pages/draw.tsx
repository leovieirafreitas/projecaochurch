...
export default function DrawPage() {
    ...
    useEffect(() => {
        if (debugInitRef.current) return;
        ...
        const addLog = (type: string, ...args: any[]) => {
            ...
            setDebugLogs(prev => [...prev.slice(-150), `[${ts}] ${type}: ${msg}`]);
        };
        ...
        window.addEventListener('error', (e) => {
            addLog('CRASH', e.message, e.filename, 'line:', e.lineno);
        });
        window.addEventListener('unhandledrejection', (e) => {
            addLog('PROMISE', e.reason?.message || e.reason);
        });

        // Initial diagnostic info
        addLog('INFO', `Draw Page loaded. UA: ${navigator.userAgent.slice(0, 80)}`);
        addLog('INFO', `Host: ${window.location.hostname}:${window.location.port || '80'}`);
        addLog('INFO', `Screen: ${screen.width}x${screen.height} | Window: ${window.innerWidth}x${window.innerHeight}`);
    }, []);

    // Log bg image changes + AUTO-TEST fetch
    useEffect(() => {
        const addLog = (type: string, msg: string) => {
            const ts = new Date().toLocaleTimeString('pt-BR');
            setDebugLogs(prev => [...prev.slice(-150), `[${ts}] ${type}: ${msg}`]);
        };
        addLog('BG', `rawBg type=${rawBg ? (rawBg.startsWith('data:') ? 'base64' : rawBg.startsWith('http') ? 'URL' : rawBg.substring(0, 30)) : 'null'}`);
        addLog('BG', `finalBgImage=${finalBgImage ? finalBgImage.substring(0, 120) : 'null'}`);
        addLog('BG', `bgColor=${bgColor}`);

        // AUTO-TEST: Tenta carregar a imagem via fetch para diagnosticar 404/CORS
        if (finalBgImage && finalBgImage.startsWith('http')) {
            fetch(finalBgImage, { method: 'HEAD' })
                .then(res => {
                    addLog('FETCH', `${finalBgImage.substring(0, 80)} → STATUS ${res.status} ${res.ok ? '✅' : '❌'} type=${res.headers.get('content-type')}`);
                })
                .catch(err => {
                    addLog('FETCH', `${finalBgImage.substring(0, 80)} → FALHA ❌ ${err.message}`);
                });
        }
    }, [rawBg, finalBgImage, bgColor]);
    ...
    useEffect(() => {
        ...
        addLog('STATE', `verseText="${(state.verseText || '').substring(0, 40)}..." ref="${state.reference}" slideIdx=${state.slideIndex}`);
        addLog('STATE', `bgVersion=${state.style?.bgVersion} | objectFit=${state.style?.objectFit || 'contain'}`);
    }, [state.verseText, state.reference, state.slideIndex, state.style?.bgVersion]);
    ...
    const handleCopyDebug = () => {
        const header = [
            ...
            `rawBg: ${rawBg ? rawBg.substring(0, 100) : 'null'}`,
            `finalBgImage: ${finalBgImage ? finalBgImage.substring(0, 120) : 'null'}`,
            `bgColor: ${bgColor}`,
            `verseText: ${(state.verseText || '').substring(0, 60)}`,
            `reference: ${state.reference}`,
            '========================',
            ''
        ].join('\n');
        const logText = header + debugLogs.join('\n');

        // iOS Safari doesn't have navigator.clipboard on HTTP, so always use textarea fallback
        try {
            ...
            if (ok) {
                ...
                alert(logText); // Último recurso: mostra em alert pra copiar manualmente
            }
        } catch {
            alert(logText);
        }
    ...
    return (
        <div className="fixed inset-0 bg-[#121212] flex flex-col font-sans select-none overflow-hidden text-white/90">
            ...
            <button
                ...
                className="fixed bottom-3 left-3 z-[99999] w-10 h-10 rounded-full flex items-center justify-center text-lg shadow-lg"
                style={{ backgroundColor: showDebug ? '#ef4444' : '#333', border: '1px solid #555' }}
            >
                🐛
            </button>
            ...
            {showDebug && (
                <div
                    className="fixed bottom-14 left-3 z-[99999] rounded-xl shadow-2xl flex flex-col"
                    style={{
                    ...
                    <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="px-2 py-0.5 text-[10px] bg-zinc-700 text-zinc-300 rounded hover:bg-zinc-600 font-mono"
                            >
                                LIMPAR
                            </button>
                            <button
                                ...
                                style={{ backgroundColor: debugCopied ? '#22c55e' : '#3b82f6', color: '#fff' }}
                            >
                                {debugCopied ? '✅ COPIADO!' : '📋 COPIAR TUDO'}
                            </button>
                        </div>
                    </div>

                    {/* Status Bar */}
                    <div className="px-3 py-1.5 border-b border-white/5 flex flex-wrap gap-x-3 gap-y-0.5 text-[9px] font-mono">
                        <span className="text-cyan-400">BG: {finalBgImage ? (finalBgImage.startsWith('data:') ? '📦 Base64' : '🔗 URL') : '❌ Sem Fundo'}</span>
                        <span className="text-yellow-400">Cor: {bgColor}</span>
                        <span className="text-purple-400">Ref: {state.reference || '—'}</span>
                        <span className="text-orange-400">Logs: {debugLogs.length}</span>
                    </div>

                    {/* Logs */}
                    <div
                        className="flex-1 overflow-y-auto px-3 py-2 font-mono text-[10px] leading-relaxed"
                        style={{ WebkitOverflowScrolling: 'touch' }}
                    >
                        {debugLogs.length === 0 ? (
                            <span className="text-zinc-600">Aguardando logs...</span>
                        ) : (
                            debugLogs.map((log, i) => (
                                <div
                                    key={i}
                                    ...
                                    style={{
                                        color: log.includes('ERR') || log.includes('CRASH') ? '#f87171' :
                                            log.includes('WARN') ? '#fbbf24' :
                                                log.includes('BG') ? '#67e8f9' :
                                                    log.includes('STATE') ? '#c084fc' :
                                                        log.includes('INFO') ? '#4ade80' :
                                                            '#a1a1aa'
                                    }}
                                >
                                    {log}
