...
export const cleanText = (text: string) => {
    if (!text) return "";
    return text.replace(/(\{[^}]+\}|<[^>]+>|\(+[HG]?\d+\)+|\[\d+\])/g, '').replace(/\s+/g, ' ').trim();
};

export const BIBLE_BOOKS_EN: Record<string, string> = {
...
export const getBookName = (bookId: string, lang?: string) => {
    if (lang && lang.toLowerCase().startsWith('en')) {
        return BIBLE_BOOKS_EN[bookId] || BIBLE_BOOKS_DATA[bookId]?.name || bookId;
    }
    return BIBLE_BOOKS_DATA[bookId]?.name || bookId;
};
...
export const NT_ONLY_VERSIONS = ['PORARA', 'PORARC', 'TYNDALE'];
...
export const BIBLE_ABBR_EN: Record<string, string> = {
    'GEN': 'Gen', 'EXO': 'Ex', 'LEV': 'Lev', 'NUM': 'Num', 'DEU': 'Deut',
    'JOS': 'Josh', 'JDG': 'Judg', 'RUT': 'Ruth', '1SA': '1Sam', '2SA': '2Sam',
    '1KI': '1Kgs', '2KI': '2Kgs', '1CH': '1Chr', '2CH': '2Chr',
    'EZR': 'Ezra', 'NEH': 'Neh', 'EST': 'Est', 'JOB': 'Job', 'PSA': 'Ps',
    'PRO': 'Prov', 'ECC': 'Eccl', 'SNG': 'Song', 'ISA': 'Isa',
    'JER': 'Jer', 'LAM': 'Lam', 'EZK': 'Ezek', 'DAN': 'Dan',
    'HOS': 'Hos', 'JOL': 'Joel', 'AMO': 'Amos', 'OBA': 'Obad', 'JON': 'Jon',
    ...
    '1TH': '1Thes', '2TH': '2Thes', '1TI': '1Tim',
    '2TI': '2Tim', 'TIT': 'Tit', 'PHM': 'Phlm', 'HEB': 'Heb',
    'JAS': 'Jas', '1PE': '1Pet', '2PE': '2Pet', '1JN': '1Jn',
    '2JN': '2Jn', '3JN': '3Jn', 'JUD': 'Jude', 'REV': 'Rev'
};

export const getBookAbbr = (bookId: string, lang?: string) => {
    if (lang && lang.toLowerCase().startsWith('en')) {
        return BIBLE_ABBR_EN[bookId] || (BIBLE_BOOKS_DATA[bookId] ? BIBLE_BOOKS_DATA[bookId].abbr : bookId);
    }
    return (BIBLE_BOOKS_DATA[bookId] ? BIBLE_BOOKS_DATA[bookId].abbr : bookId);
};
